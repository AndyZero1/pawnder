            ...
            content: SizedBox(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    clinicData['detalii']!.isEmpty ? 'No details provided.' : clinicData['detalii']!,
                ...
                onPressed: () async {
                  if (textController.text.isNotEmpty && locationId != null) {
                    ...
                    try {
                      ...
                      final token = prefs.getString('jwt_token'); // Corectat numele token-ului
                      final apiUrl = 'http://127.0.0.1:8000/api/map/clinic/$locationId/add-review/'; // Corectat IP-ul local
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Text(telefon.isEmpty ? 'Not specified' : telefon, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              ],
            ),
          ],
        ),
          ...
          ElevatedButton.icon(
            onPressed: () async {
              ...
              try {
                ...
                await http.post(
                  Uri.parse('http://127.0.0.1:8000/api/events/$eventId/join/'),
                  headers: {
                ...
                print("Eroare la join din harta: $e");
              }
            ...
            child: ElevatedButton(
              onPressed: () async {
                 ...
                 try {
                    final prefs = await SharedPreferences.getInstance();
                    final token = prefs.getString('jwt_token'); // Corectat numele token-ului
                    final apiUrl = 'http://127.0.0.1:8000/api/map/gem/$locationId/claim/'; // Corectat IP-ul local
                    ...
                    if (response.statusCode == 200) {
                      Navigator.of(context).pop(true); 
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Treasure claimed successfully!')));
                    } else {
                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error: ${response.statusCode}')));
                    }
                 } catch (e) {
              ...
              child: const Text('Collect Prize', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
            ),
          ),
        ],
      );
